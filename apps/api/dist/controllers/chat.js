...
import { getConversationHistoryService, getConversationMessagesService, deleteConversationById } from "../service/chats.js";
...
chatRouter.post("/messages", rateLimit, async (c) => {
    ...
    return stream(c, async (s) => {
        ...
        try {
            const maybe = result.stream;
            const textIter = (maybe && typeof maybe[Symbol.asyncIterator] === "function")
                ? maybe
                : maybe.textStream;
        ...
        try {
            await saveAgentResponse(result.conversationId, fullResponse, result.agentType);
        }
        catch (err) {
            try {
                await s.write(JSON.stringify({ type: "error", message: err.message }) + "\n");
            }
            catch { }
        }
...
chatRouter.get("/conversations/:id", rateLimit, async (c) => {
    ...
    if (!parsed.success)
        return c.json({ error: "id param is required" }, 400);
